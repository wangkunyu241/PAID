...
from robustbench.model_zoo.our_vit import ViT_Ours
import timm
...
class DORA_ALIGN(nn.Module):
    def __init__(self, model, optimizer, steps=1, episodic=False, lamda=1.0):
        ...
        self.model_state, self.optimizer_state = copy_model_and_optimizer_align(self.model, self.optimizer)
    ...
    def reset(self):
        ...
        load_model_and_optimizer_align(self.model, self.optimizer, self.model_state, self.optimizer_state)
        self.model_state, self.optimizer_state = copy_model_and_optimizer_align(self.model, self.optimizer)

    @torch.enable_grad()
    def forward_and_adapt(self, x, model, optimizer):
        features = self.model.forward_raw_features(x)
    ...
    def obtain_src_stat(self, data_path, num_samples=3000):
        num = 0
        features = []
        net = ViT_Ours().eval().cuda()
        data_config = timm.data.resolve_model_data_config(net.vit)
        src_transforms = timm.data.create_transform(**data_config, is_training=False)
        src_dataset = ImageFolder(root=f"{data_path}/train", transform=src_transforms)
        src_loader = torch.utils.data.DataLoader(src_dataset, batch_size=64, shuffle=True)
        
        with torch.no_grad():
            for _, dl in enumerate(src_loader):
                images = dl[0].cuda()
                feature = net.forward_raw_features(images)
                output = net.vit.forward_head(feature)
                ent = softmax_entropy_align(output)
...
def configure_model_qkvom(model, cfg):
    for param in model.parameters():
        param.requires_grad = False
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = model.cpu()
    hra_params, names = inject_hra_qkvom(model, cfg.OPTIM.RANK)
    model.to(device)
    model.train()
    for p in hra_params:
...
def configure_model_qkvom_align(model, cfg):
    ...
    if cfg.TEST.ckpt != None:
        ...
        model.vit.load_state_dict(checkpoint, strict=True)
...
def inject_hra_qkvom(model: nn.Module, r: int = 8) -> Tuple[List[nn.Parameter], List[str]]:
    ...
    for module_name, module in model.named_modules():
        ...
        for block_id in [10, 11, 12]:
            if f'blocks.{block_id - 1}.' in module_name:
                skip = True
                break
        if skip:
            continue

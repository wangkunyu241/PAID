...
def pretrain(description):
    ...
    if cfg.MODEL.ADAPTATION == "cifar10_pretrain":
        logger.info("cifar10 pretrain start")
        model, hra_params = dora_magnitude_angle_hra.configure_model_qkvom_align(base_model, cfg)
        # Prepare dataset
        trainset = torchvision.datasets.CIFAR10(root='/data/asdf95xx/CIFAR10/cifar10', train=True, download=False, transform=transform_train)
        trainloader = torch.utils.data.DataLoader(trainset, batch_size=64, shuffle=True, num_workers=4)

        testset = torchvision.datasets.CIFAR10(root='/data/asdf95xx/CIFAR10/cifar10', train=False, download=False, transform=transform_test)
        ...
        weights_path = './output/cifar10_pretrain.pth'
